...
public:
    ...
    // StackChan v10.10.7: apagado DEFINITIVO que cancela la animación.
    // setColor() usa move() (fade 0.3s): si la animación queda viva,
    // update() repinta el color por encima de cualquier showRgbColor(0,0,0).
    // teleport(0,0,0) pone negro al instante y done()=true -> update()
    // ya no vuelve a pintar.
    void turnOff()
    {
        _color_anim.teleport(0, 0, 0);
        _snap_to_target_on_rest = false;
        for (int i = 0; i < _led_count; i++) {
            set_rgb_color_impl(i, 0, 0, 0);
        }
        refresh_rgb_impl();
    }
