...
ShyDecorator::ShyDecorator(lv_obj_t* parent, uint32_t destroyAfterMs)
{
    ...
    // StackChan v10.13 (26/08/2026): subir el rubor a la primera capa para que
    // la cara (ojos/boca) no lo tape al redibujarse. Fix del rubor invisible
    // cuando lo lanza el server vía ShowHeart (en la caricia sí se veía
    // porque la cara no se redibujaba después).
    _left->moveForeground();
    ...
    // StackChan v10.13 (26/08/2026): rubor a la primera capa (ver izquierda).
    _right->moveForeground();
