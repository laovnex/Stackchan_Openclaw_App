...
private:
    ...
    int head_pet_modifier_id_           = -1;
    int imu_modifier_id_                = -1;
...
public:
    ...
    // StackChan v10.9.9: tools display del gateway (self.display.*)
    void SetAvatarFace(const char* face);
    void SetMouthShape(const char* shape);
    void SetBlinkEnabled(bool enabled);

    // StackChan v10.11 (26/08/2026): decoración de corazón invocable desde el
    // tracker de emociones del ai-server (self.display.show_heart).
    // Muestra el HeartDecorator durante duration_ms (default 2500ms).
    void ShowHeart(int duration_ms);
