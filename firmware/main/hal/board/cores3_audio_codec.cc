...
CoreS3AudioCodec::CoreS3AudioCodec(void* i2c_master_handle, int input_sample_rate, int output_sample_rate,
    ...
    // StackChan v10.10.9: pa_gain -6 -> -4. Con -6 el codec va a +9.61 dB y
    // los picos del archivo caliente (TP -1 dB) RECORTAN en las sílabas
    // fuertes -> petardeo aleatorio (el usuario 25/08 17:15). Con -4:
    // hw_gain = -3.61 + (-4) = -7.61 -> codec a +7.61 dB: mismo nivel
    // medio percibido, picos con 2 dB más de margen.
    aw88298_cfg.hw_gain.pa_gain = 1.0;
