...
static void _stackchan_update_task(void* param)
{
    ...
    // StackChan v10.9.6: esperar a que el chat IA arranque de verdad (modo xiaozhi).
    // Evita que la task se suicide en el instante entre su creación y el arranque.
    while (!hal_bridge::is_xiaozhi_mode()) {
        vTaskDelay(pdMS_TO_TICKS(20));
    }

    while (1) {
        // StackChan v10.9.6: si el chat terminó (home) y volvimos al launcher,
        // morir ANTES de que main.cpp destruya mooncake/LVGL. La task zombie
        // tocaba widgets LVGL ya liberados -> use-after-free -> panic -> reboot.
        if (!hal_bridge::is_xiaozhi_mode()) {
            mclog::tagInfo("hal", "stackchan update task exit (chat done)");
            vTaskDelete(NULL);
        }
        ...
        // Re-check justo antes de tocar LVGL (ventana de riesgo minima)
        if (!hal_bridge::is_xiaozhi_mode()) {
            mclog::tagInfo("hal", "stackchan update task exit (chat done, pre-lvgl)");
            vTaskDelete(NULL);
        }
        ...
        if (!is_setup_done) {
            ...
            GetHAL().startSntp();
            // StackChan v10.5: home en modo chat -> salir del chat y volver al
            // launcher SIN reiniciar el dispositivo (antes hacia esp_restart).
            view::create_home_indicator(
                [&]() {
                    mclog::tagInfo(_tag, "home pressed in chat mode -> back to launcher");
                    hal_bridge::request_exit_chat();
                },
                0x81DBBD, 0x134233);
