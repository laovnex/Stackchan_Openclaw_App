...
public:
    ...
    void init()
    {
        // StackChan v10.5: normalizar el esquema de la URL. La config de fábrica
        // guarda http://47.113.125.164:12800 pero un WebSocket necesita ws://
        // (o wss:// para https). Sin esto la app avatar no conecta a la nube.
        std::string server_url = secret_logic::get_server_url();
        ...
        if (server_url.rfind("https://", 0) == 0) {
            replace_all(server_url, "https://", "wss://");
        } else if (server_url.rfind("http://", 0) == 0) {
            replace_all(server_url, "http://", "ws://");
        }
        _url = fmt::format("{}/stackChan/ws?deviceType=StackChan", server_url);
    ...
    void connect()
    {
        // StackChan v10.7: token RSA-MAC según la documentación oficial del server
        // (github.com/m5stack/StackChan README, sección Device Workflow):
        //   plainText = "MAC|nonce|unix_timestamp", cifrado RSA-OAEP-SHA256 con
        //   la clave pública del server, Base64 -> header Authorization.
        // VERIFICADO contra la nube china (47.113.125.164:12800) el 24/08 23:28
        // desde el Mac: OAEP-SHA256 MAC|nonce|ts -> conexión ACEPTADA.
        // La MAC del bicho (68:ee:8f:d7:4c:d0) está registrada en la nube.
        std::string token = secret_logic::generate_mac_auth_token(SystemInfo::GetMacAddress());
        if (token.empty()) {
            // Fallback: token débil (no debería ocurrir si la clave pública embebida es válida)
            token = secret_logic::generate_auth_token();
        }
        ...
        // StackChan v10.9.2: Device-Id con la MAC COMPACTA (sin separadores),
        // igual que el token. La app M5Stack busca al bicho por la MAC
        // compacta que dejó el bind de fábrica en la cuenta (68ee8fd74cd0);
        // si el header lleva la MAC con dos puntos, el server no lo asocia.
        _websocket->SetHeader("Device-Id", secret_logic::mac_compact(SystemInfo::GetMacAddress()).c_str());
        _websocket->SetHeader("Client-Id", Board::GetInstance().GetUuid().c_str());
        ...
        _websocket->OnDisconnected([this]() {
            ...
            // StackChan v10.10.8: apagar LEDs AL CORTARSE la conexión. El log de
            // el usuario (25/08) demostró que el server pinta azul (speaking) al
            // procesar la última orden y al cerrarse el WS nadie apagaba ->
            // LED azul colgado. Aquí es EL punto correcto: el firmware sabe
            // que la conversación terminó. set_leds_off() respeta el color
            // manual persistido (tool del LLM) y cancela la animación.
            hal_bridge::set_leds_off();
