...
void AudioService::Deinit() {
    // StackChan v10.6: liberar TODOS los recursos pesados al salir del chat.
    // Sin esto, el launcher se quedaba con ~28-39KB libres y se reiniciaba
    // a los 2 movimientos por el menu (RAM sin liberar del motor de voz).
    Stop();

    if (wake_word_) {
        // NO resetear: la siguiente Initialize() reutiliza el objeto y
        // EnableWakeWordDetection(true) lo re-inicializa si hace falta.
        wake_word_->Stop();
    }
    if (audio_processor_) {
