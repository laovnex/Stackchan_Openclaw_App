...
export function startDeviceControlServer(port: number, host = '127.0.0.1'): void {
    ...
    const server = http.createServer(async (req, res) => {
        ...
        if (req.method !== 'POST' || (pathname !== '/tools/call' && pathname !== '/internal/followup' && pathname !== '/internal/abort')) {
        ...
        try {
            ...
            if (pathname === '/internal/abort') {
                activeSession.abort()
                // StackChan v10.9: cierre real estilo app china ("desconecta").
                // Tras abortar, cerramos el WebSocket del dispositivo para que
                // el firmware reaccione igual que con la nube china:
                // OnDisconnected -> idle + LOW_POWER + micro apagado.
                // El delay corto deja terminar el flush del TTS.
                setTimeout(() => {
                    activeSession?.closeDeviceConnection()
                }, 800)
                sendJson(res, 200, { success: true, result: { aborted: true } })
                return
            }
