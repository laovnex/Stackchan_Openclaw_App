...
export const tools: ToolDefinition[] = [
    ...
    {
        name: 'stackchan_abort',
        description: [
            'Stop the StackChan conversation immediately: cancel listening, stop TTS, clear queued follow-ups, leave the robot idle.',
            'Use when the user asks to stop, disconnect, silence, or shut up.',
        ].join(' '),
        inputSchema: {
            type: 'object',
            properties: {},
...
async function callControl(path: string, payload: Record<string, unknown>): Promise<unknown> {
...
export async function handleRequest(req: JsonRpcRequest): Promise<void> {
    ...
    if (req.method === 'tools/call') {
        ...
        try {
            ...
            if (name === 'stackchan_abort') {
                const result = await callControl('/internal/abort', {})
                respond(req.id, {
            ...
            const result = await callControl('/tools/call', { name, args })
            respond(req.id, {
